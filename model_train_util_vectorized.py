...
def eval_model(model,data_loader):
    model.eval()
    model.set_adj_test_matrix(True)
    all_preds=[]
    with torch.no_grad():
        for _ in range(10):
            preds= []
            ...
            for edges in data_loader:
                ...
                try:
                    hidden_vec = model(edges[0],train_stage = False)
                    label = edges[0][:,-1]
    ...
    all_preds = np.mean(np.asarray(all_preds),axis=0)
    labels = np.asarray(labels)
    rec = recall_at_90_precision(all_preds, labels)
    model.set_adj_test_matrix(False)
    model.train()
    return rec
    ...
    for i in range(epoch):
        ...
        for index, d in enumerate(tr_data_loader):
            try:
                ...
                val = -1.*alpha* torch.log(1 - hidden_vec[label == 0]) -1.*(1-alpha)*torch.log(hidden_vec[label == 0])
